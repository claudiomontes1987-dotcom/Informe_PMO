document.addEventListener('DOMContentLoaded', () => {
    const plantItems = Array.from(document.querySelectorAll('.plant-list li'));
    const informeBody = document.getElementById('informe-body');
    ...
    const enhanceContent = () => {
        informeBody.querySelectorAll('table').forEach((table) => {
            if (!table.parentElement.classList.contains('table-wrapper')) {
                const wrapper = document.createElement('div');
                wrapper.className = 'table-wrapper table-scroll';
    ...
    const setActive = (item) => {
        ...
        plantItems.forEach((li) => li.classList.remove('active'));
        item.classList.add('active');

        const informeId = item.dataset.informe;
        const pdfPath = item.dataset.pdf;
        const label = item.dataset.label || item.textContent.trim();

        informeBody.innerHTML = informes[informeId] || '<p>No hay información disponible.</p>';
        enhanceContent();

        if (pdfPath) {
            pdfFrame.src = pdfPath;
            pdfFilename.textContent = pdfPath.split('/').pop();
            downloadPdfLink.href = pdfPath;
            downloadPdfLink.setAttribute('download', pdfPath.split('/').pop());
            openPdfBtn.onclick = () => window.open(pdfPath, '_blank');
        }

        plantName.textContent = label;
